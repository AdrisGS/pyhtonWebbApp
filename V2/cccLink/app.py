...
import static.environmentVariables as enVar

app = Flask(__name__)
database = MySQL()

app.config['MYSQL_DATABASE_USER'] = enVar.dbRoute
app.config['MYSQL_DATABASE_PASSWORD'] = enVar.dbPassword
app.config['MYSQL_DATABASE_DB'] = enVar.dbName
app.config['MYSQL_DATABASE_HOST'] = enVar.dbHost

# app.secret_key = enVar.key
...
def add_paymentInfo():
    if request.method == 'POST':
        ...
        result = 'OK'
        link_id = id_generator().url_id(str(order_id))
        link_url = enVar.domain_address+link_id
...
def subscription_create():
  conekta.api_key = enVar.conekta_key
...
if __name__ == '__main__':
    app.run(port = enVar.api_port, debug = enVar.debug)